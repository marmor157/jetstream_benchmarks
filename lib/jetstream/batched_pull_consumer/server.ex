defmodule Jetstream.BatchedPullConsumer.Server do
  @moduledoc false
  ...
  alias Jetstream.BatchedPullConsumer.ConnectionOptions
  ...
  def connect(
    ...
    with {:ok, conn} <- connection_pid(connection_name),
         ...
         :ok <- initial_pull(conn, stream_name, consumer_name, listening_topic, state.batch_size),
  ...
  def handle_info(
    ...
    spawn(fn ->
      Jetstream.BatchedPullConsumer.Job.run(message, module, listening_topic, state)
    end)
  ...
  def handle_info(
    ...
    Logger.debug(
      ...
      NATS connection has died. BatchedPullConsumer is reconnecting.
      """,
      module: module,
      listening_topic: listening_topic,
      subscription_id: subscription_id,
  ...
  defp initial_pull(conn, stream_name, consumer_name, listening_topic, batch_size \\ 128) do
    Gnat.pub(
      conn,
      "$JS.API.CONSUMER.MSG.NEXT.#{stream_name}.#{consumer_name}",
