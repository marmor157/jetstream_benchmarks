defmodule Jetstream.BatchedPullConsumer do
  @moduledoc """
  ...
  is asked for them.
  ## Example
  Declare a module which uses `Jetstream.BatchedPullConsumer` and implements `c:init/1` and
  `c:handle_message/2` callbacks.
      defmodule MyApp.BatchedPullConsumer do
  ...
  * `:connection_retry_timeout` - a duration in milliseconds after which the BatchedPullConsumer which
    failed to establish NATS connection retries, defaults to `1000`
  * `:connection_retries` - a number of attempts the BatchedPullConsumer will make to establish the NATS
  ...
  * `:inbox_prefix` - allows the default `_INBOX.` prefix to be customized. Should end with a dot.
  ...
  init argument:
      defmodule MyApp.BatchedPullConsumer do
        use Jetstream.BatchedPullConsumer
        def start_link() do
          Jetstream.BatchedPullConsumer.start_link(__MODULE__, %{counter: counter})
        end
        @impl true
  ...
  ## How to supervise
  A `BatchedPullConsumer` is most commonly started under a supervision tree. When we invoke
  `use Jetstream.BatchedPullConsumer`, it automatically defines a `child_spec/1` function that allows us
  ...
  for example:
      def start_link(opts) do
        ...
        Jetstream.BatchedPullConsumer.start_link(__MODULE__, initial_state, opts)
      end
  ...
  `use Jetstream.BatchedPullConsumer` also accepts a list of options which configures the child
  ...
  For example:
      use Jetstream.BatchedPullConsumer, restart: :transient, shutdown: 10_000
  See the "Child specification" section in the `Supervisor` module for more detailed information.
  The `@doc` annotation immediately preceding `use Jetstream.BatchedPullConsumer` will be attached to
  ...
  * `:noreply` - nothing is sent. You may send later asynchronously an ACK or NACK message using
  ...
  defmacro __using__(opts) do
    quote location: :keep, bind_quoted: [opts: opts] do
      @behaviour Jetstream.BatchedPullConsumer
  ...
  def start_link(module, init_arg, options \\ []) when is_atom(module) and is_list(options) do
    Connection.start_link(
      Jetstream.BatchedPullConsumer.Server,
  ...
  @doc """
  Starts a `Jetstream.BatchedPullConsumer` process without links (outside of a supervision tree).
  ...
  ## Example
      {:ok, consumer} =
        BatchedPullConsumer.start_link(ExamplePullConsumer,
          connection_name: :gnat,
          stream_name: "TEST_STREAM",
          consumer_name: "TEST_CONSUMER"
        )
      :ok = BatchedPullConsumer.close(consumer)
  """
